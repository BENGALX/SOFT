...
class CHANNELSMod(loader.Module):
    ...
    async def send_manual_message(self, text):
        ...
        manual_part1 = (
            ...
            f"После установки модуля вам нужно разделить все аккаунты на условные группы (по умолчанию стоит 0) и на одном из аккаунтов каждой группы включить логгирование (по умолчанию выключено). Для облегчения задачи мы используем формат групп как на сервере (15-20 аккаунтов) и включаем логи только на аккаунтахъ с панелькой (управляющих). Это сделано чтобы не перенагружать сервера используя задержку между действиями каждой группы в Х*20 секунд.\n"
        )
        ...
        if parts[1] == f"@{user.username}":
            ...
            await self.client.send_file(self.config["ownerchat"], manual_part2)
            await asyncio.sleep(2)
            await self.client.send_file(self.config["ownerchat"], manual_part3)
