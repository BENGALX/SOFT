...
class CHANNELSMod(loader.Module):
    ...
    async def send_manual_message(self, text):
        ...
        manual_part1 = (
            ...
            f"Без них он тоже будет работат если что.\n"
            f"Для начала нужно разделить все ваши аккаунты на условные группы (по умолчанию стоит группа 1). Это сделано чтобы не перенагружать сервера используя задержку между действиями каждой группы в Х*20 секунд.\n"
            f"Для упрощения мы используем размер групп как на сервере (по 15-20 аккаунтов)."
            f"Далее на одном из аккаунтов каждой группы включить логгирование (по умолчанию выключено).\n"
            f"и включаем логи только на аккаунтахъ с панелькой (управляющих)."
            f""
            f""
            f""
            f""
        ...
        manual_part2 = (
            ...
            f"group — номер сервера или группы аккаунтов, определяющий задержку.\n"
        ...
        manual_part3 = (    
            f"<b>🔗1. Подписки:</b>\n"
            f"/sub [target]\n"
            f"Для публичных: https://t.me/, //t.me/ или @\n"
            f"Для частных: https://t.me/+, t.me/+\n"
    
            f"<b>🔗2. Отписки:</b>\n"
            f"/uns [target]\n"
            f"Для публичных: аналогично подпискам (ссылки и теги).\n"
            f"Для частных: ID в формате 100... (без минуса).\n"
        )

        image_url = "https://raw.githubusercontent.com/BENGALX/SOFT/bengal/IMAGE/BENGAL.jpg"
        user = await self.client.get_me()
        if parts[1] == f"@{user.username}":
            await self.client.send_file(self.config["ownerchat"], image_url, caption=manual_part1)
            await asyncio.sleep(2)
            await self.client.send_message(self.config["ownerchat"], manual_part2)
            await asyncio.sleep(2)
            await self.client.send_message(self.config["ownerchat"], manual_part3)
