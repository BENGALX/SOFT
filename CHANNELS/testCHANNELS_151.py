...
class CHANNELSMod(loader.Module):
    ...
    async def send_manual_message(self, text):
        ...
        manual_intro = (
            ...
            "<b>🔗 Справка:</b> Используйте кнопки ниже для просмотра мануала.\n\n"
        )
        buttons = [
        ...
        if parts[1] == f"@{user.username}":
            await self.client.send_file(self.owner_chat, image_url, caption=manual_intro, buttons=buttons, parse_mode='html')
    ...
    async def client_ready(self, client, db):
        ...
        async def manual_callback(event):
            """Обработка инлайн-кнопок для мануала"""
            if event.data == b'setup_manual':
                text = (
            ...
            elif event.data == b'config_manual':
                text = (
                    f"<b>🔗 Конфигурация:</b>\n"
