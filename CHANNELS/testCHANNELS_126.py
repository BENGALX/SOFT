...
class CHANNELSMod(loader.Module):
    ...
    async def send_manual_message(self, text):
        ...
        manual_text = (
            ...
            f"<b>2. Отписки:</b>\n"
            f"— /uns [ссылка/тег/ID] — Отписаться от канала или группы.\n"
            f"    Доступные форматы:\n"
            f"    - Для публичных каналов:\n"
            f"        - Теги: @channel_name\n"
            ...
            f"    - ownerchat: ID чата владельца для логирования.\n"
            f"    - logger: Включить или отключить логирование (True/False).\n"
            f"    - group: Номер группы, определяющий задержку.\n"
            f"    Возможные аргументы:\n"
            f"    - Для ownerchat: любой действительный ID чата.\n"
            f"    - Для logger: 1 (включить) или 0 (выключить).\n"
            f"    - Для group: целое число.\n\n"

            f"<b>4. Справка:</b>\n"
            f"— /manual @user — Справка по командам модуля.\n"
            f"    Отправляет подробную информацию о командах пользователю, указанному в аргументе.\n"
        )
