...
class BENGALSOFTMod(loader.Module):
    ...
    async def send_manual_message(self):
        ...
        try:
            manual_basic = (
                "<b>⚙️ Базовая настройка:</b>\n"
                "▪️Для начала нужно разделить все аккаунты на виртуальные группы (изначально стоит 1). "
                "В каждую добавляем до 5-10 акков. Группа это просто множитель задержки в 20 секунд.\n"
                "▪️Далее на одном из акков каждой группы нужно включить логгирование (по умолчанию оно выключено)."
                "Не путайте группу (объединение, их много) с группой (чат, у нас он один).\n\n"
                "<b>⚙️ Команда настройки:</b>\n"
                "/config set [p] [nv] [us]\n"
                "▪️[p] — имя переменной\n"
                "▪️[nv] — новое значение\n"
                "▪️[us] — @ (1 | много | all)\n"
